...
def test_execution_client_parses_holdings_position_envelope():
    client = ExecutionClient(settings=Settings(execution_base_url="http://testserver"))
    client._get_json = lambda paths: {  # type: ignore[method-assign]
        "holdings": [
            {
                "symbol": "AAPL",
                "asset_class": "stock",
    ...
    assert positions[0].market_value == 19_000.0


def test_execution_client_rejects_unknown_position_envelope():
    client = ExecutionClient(settings=Settings(execution_base_url="http://testserver"))
    client._get_json = lambda paths: {"holdings_count": 1}  # type: ignore[method-assign]
    try:
        with pytest.raises(ExecutionClientError, match="missing expected list field"):
