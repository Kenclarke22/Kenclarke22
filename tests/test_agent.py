...
def test_limit_orders_require_positive_limit_price():
...
def test_rebalance_does_not_emit_zero_dollar_limits():
    strategy = RebalanceStrategy()
    ctx = StrategyContext(
        account=AccountSnapshot(equity=100.0),
        positions=[
            Position(
                symbol="PENNY",
                asset_class=AssetClass.STOCK,
        ...
        market_quotes={"PENNY": 0.004},
        metadata={"target_weights": {"PENNY": 0.0}, "rebalance_threshold": 0.001},
    )
