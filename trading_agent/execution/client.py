...
class ExecutionClient:
    ...
    def submit_order(self, intent: OrderIntent) -> OrderResponse:
        ...
        response = self._parse_order_response(data)
        self._validate_order_submission(response)
        return response
    ...
    def _validate_order_submission(self, response: OrderResponse) -> None:
        if response.status in {OrderStatus.REJECTED, OrderStatus.CANCELLED}:
            detail = f": {response.message}" if response.message else ""
            raise ExecutionClientError(
                f"Order submission returned {response.status.value}{detail}",
                body=response.raw,
            )
        if not response.id:
            raise ExecutionClientError("Order submission response missing order id", body=response.raw)
