...
class ExecutionClient:
    ...
    def get_orders(self, status: str | None = None) -> list[OrderResponse]:
        ...
        items = self._extract_collection(data, keys=("orders", "data"), label="orders")
    ...
    def _extract_collection(self, data: Any, *, keys: tuple[str, ...], label: str) -> list[Any]:
        if isinstance(data, list):
            return data
        if not isinstance(data, dict):
            raise ExecutionClientError(f"{label.title()} response must be a list or object")
        ...
        for key in keys:
            ...
            joined = ", ".join(keys)
            raise ExecutionClientError(f"{label.title()} response missing expected list field ({joined})")

        if not isinstance(items, list):
            raise ExecutionClientError(f"{label.title()} field must be a list")
        return items
